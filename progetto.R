  ...
  par(mfrow=c(3,2))
  hist(dice.data$mov_mean, main="Media Movimento", 
       breaks = 10, col="green", xlab = "media")
  hist(dice.data$mov_var, main="Varianza Movimento",
       breaks = 10, col="darkgreen", xlab = "varianza")
  hist(dice.data$coin_mean, main="Media coins",
       breaks = 10, col="yellow", xlab = "media")
  hist(dice.data$coin_var, main="Varianza coins",
       breaks = 10, col="gold", xlab = "varianza")
  hist(dice.data$mov_variety, main="varietà movimento",
       breaks = 7, col="red", xlab = "varietà", xlim = c(1,5))
  par(mfrow=c(1,1))
  ...
  #considerati tutti i possibili trade-off, elimino quelli che hanno una correlazione
  #positiva o negativa con dei trade-off "fondamentali"
  ...
  colnames(levels.data) <- c("character","high.mov_meanXhigh.mov_var", "high.mov_meanXlow.mov_var",
   ...
   #plot dei trade-off che hanno passato la scrematura
    ...
    trade.off.12 <- psel(dice.data, low(coin_var) * high(mov_variety), top = nrow(dice.data))
      ...
      #dal dataset games.data estraggo le frequenze di vittoria per ogni personaggio
      #e definisco la 'performance' come 1/(freq. vit + 1).
      ...
      boxplot(performance.data$freq, col=c("red"), main="vittorie/presenze")
    ...
    #per ogni personaggio, media e deviazione standard dei suoi livelli
    ...
    par(mfrow=c(2,1))
    hist(ch.levels.mean, main= "media livello per personaggio", 
         xlab= "mean", col= "green")
    plot(density(ch.levels.mean), main = "densità")
    hist(ch.levels.std, main= "deviazione standard livelli per personaggio", 
         xlab= "standard deviation", col= "darkgreen")
    plot(density(ch.levels.std), main = "densità")
    hist(levels.data$performance, main= "'performance' per personaggio", 
         xlab= "performance", col= "lightblue")
    plot(density(levels.data$performance), main = "densità")
    par(mfrow=c(1,1))
    
    #modello di regeressione: variabile dipendente <- performance
      # variabili indipendenti <- livelli
    #partendo dal modello con tutti i livelli, procedo per backward elimination
    #eliminando ogni volta la variabile con p-value maggiore
    ...
    #m0 è il modello che avrei se da m7 togliessi quelle variabili che in m1
    #avevano p-value > 0.8. Si nota che è nettamente peggiore di m7.
    m0.formula <- performance ~ high.mov_meanXhigh.mov_var + high.mov_meanXlow.mov_var +
      high.mov_meanXhigh.coin_mean + high.mov_meanXhigh.mov_variety
    m0 <- lm(formula = m0.formula, data = levels.data[,-1])
    s0 <- summary(m0)
    print(s0)
    
    #i 6 trade-off del modello finale
    ...
    legend("topleft", legend= c("modello","actual"), col=c('black','red'), pch=1)
